...
function checkGoal() {
    // For the left goal, check if the puck’s left edge has passed x = 0
    // For the right goal, check if the puck’s right edge has passed GAME_WIDTH
    if (
...
// Game update loop
setInterval(() => {
    // Update puck position if it's not held
    if (!puck.heldBy) {
        puck.x += puck.vx;
        puck.y += puck.vy;
        ...
        // First, check for a goal BEFORE applying wall bounce logic
        if (!checkGoal()) {
            // Bounce off left/right walls (if not scoring) and top/bottom walls
            ...
            // Check for puck possession if no goal was scored
            checkPuckPossession();
        }
    } else {
    ...
    for (let id in players) {
        ...
        for (let otherId in players) {
            ...
            if (distanceBetweenPlayers < minDistance) {
                ...
                // Emit updated positions for both players
